...
After entering a few logins and monitoring the network tab in `devtools`, i got suspicious that nothing was appearing meaning the logins are all handled locally within the website instead of being confirmed within a server, so i checked debugger -> sources in dev tools and looked through the `JavaScript` code for `dev.js` and found this: 

![Image2](images/image2.png)

So i just copied the devNotes directory and put it in the url bar.
...
So to locate the uncommon port i did a bigger scan of the all ports with this nmap command: `nmap -sV -p- -v [target_ip]`

I ended up finding it on port: `37370/tcp open  ftp     vsftpd 3.0.3`

I could then use the command: `ftp [target_ip] 37370`
...
After going back a directory i find a suspicious file named `valleyAutheticator` that sticks out. When i `cat` it, its encoded, when i `strings` it i dont have auth and when i `nano` it i dont have auth, so i start a http server within that directory by using this command `python3 -m http.server` then i go to my kali session and download it with `wget [target_ip]:8000/valleyAuthenticator` 
...
Unfortunately I cant write to this script. But after using `cat` on the python script being run i found out that it imports a library called base64 which after using this command i found out i can write to it:

![Image9](images/image9.png)
 
The next part was simple, all I did was import os and put a reverse shell in the library and set up a listener on my attacking machine with `netcat` and I got access to the root user:
