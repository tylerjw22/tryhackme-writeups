...
I searched up what is this file and the first result came up with a CVE (good news)
...
I then downloaded the exploit py file from the exploitdb page (https://www.exploit-db.com/exploits/49909) and asked claude a few questions to better understand how it works and how to use it and eventually i was able to run this command:
...
(i also ran out of time in the THM VM so the target machine has a different ip address than before)
...
This command finds files from the root folder owned by the user lucien and puts all the errors in /dev/null.
...
After some enumeration i found the password in the .bash_history file from lucien when they accidentally typed it: with this command: cat /home/lucien/.bash_history
...
So we set up the listener like this: nc -lvnp 4444
...
The ctf writeup i was reading said they used a program called pspy64 that found a program being run every so often by the user morpheus. But I couldn't find it. 
